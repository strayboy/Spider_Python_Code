...
for root,dirs,files in os.walk(path):
	for file in files:
		if (file.split('.')[-1]=='txt' or file.split('.')[-1]=='TXT') and file[7]=='1':
			filenames.append(file)
print filenames

f=open("wa_gr10120170222.txt","rU")
table=0
unit=""
end=0
head=[]
	...
	if table==0:
		...
		if index==1:
			...
			for i in range(len(data[1:])):
				...
				print date
				record=(data[0].strip(),publish_date,head[1:][i].strip(),unit,int(data[1:][i].replace('*',"").replace(',',"").strip()),updatetime)
				print record
				index=index+1
		...
		if ' REPORTED IN WEEK ENDING' in line:
			...
			if len(result)==0:
				...
				for pair in result:
					...
					value=re.findall(reg2,pair)[0].replace(",","").strip()
					table2_lines.append((datetime.date(2016,1,1),cur_portarea,area,unit,value,datetime.datetime(2016,1,1)))
		elif '-------------------------------------------' in line:
			end=end+1
			index=index+1
			if end==2:
		...
		rc=[]
		publish_date=datetime.date(2017,2,21)
