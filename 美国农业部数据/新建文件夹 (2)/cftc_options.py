...
class cftcSpider(scrapy.Spider):
	...
	def parse(self,response):
		...
		time.sleep(3)
		file=browser.find_element_by_xpath("//*[@id='box-text_image']/div/div/ul/li/table[2]/tbody/tr[1]/td[1]/p/a[2]")
		#file.click()
		...
		try:
			...
			print publish_time
			sql="select Top 1 * from Cftc_FuturesAndOptions where As_of_Date_In_Form_YYMMDD = "+sign
			cur.execute(sql)
			result=cur.fetchall()
			if len(result)==0:
				strl="insert into Cftc_FuturesAndOptions values("
				for i in range(188):
					strl=strl+"%s,"
			...
			if(str(description)==None):
				description="Cftc_FuturesAndOptions success;"+filename+";"
			else:
				description=str(description)+"Cftc_FuturesAndOptions success;"+filename+";"
			...
			description=cbot[5]
			if(str(description)==None):
				description="Cftc_FuturesAndOptions file error;"+filename+";"
			else:
				description=str(description)+"Cftc_FuturesAndOptions file error;"+filename+";"
	...
	def readexcel(self,filename):
		...
		f.extractall()
		f.close()
		data=xlrd.open_workbook('c_year.xls')
		table=data.sheets()[0]
		lexcel=[]
		...
		if os.path.exists('c_year.xls'):
			os.remove('c_year.xls')
