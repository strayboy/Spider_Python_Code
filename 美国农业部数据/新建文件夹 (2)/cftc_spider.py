...
class cftcSpider(scrapy.Spider):
	...
	def parse(self,response):
		...
		while True:
			try:
				...
				time.sleep(3)
				file=browser.find_element_by_xpath("//*[@id='box-text_image']/div/div/ul/li/table[1]/tbody/tr[1]/td[1]/p/a[2]")
				#file.click()
		...
		try:
			...
			print publish_time
			sql="select Top 1 * from Cftc_Futures where As_of_Date_In_Form_YYMMDD = "+sign
			cur.execute(sql)
			result=cur.fetchall()
			if len(result)==0:
				str1="insert into Cftc_Futures values("
				for i in range(188):
					str1=str1+"%s,"
				
				str1=str1[:-1]
				str1=str1+")"
				cur.executemany(str1,lexcel)
			...
			if(str(description)==None):
				description="cftc success;"+filename+";"
			else:
				description=str(description)+"cftc success;"+filename+";"
			...
			description=cbot[5]
			if(str(description)==None):
				description="cftc file error;"+filename+";"
			else:
				description=str(description)+"cftc file error;"+filename+";"
	...
	def readexcel(self,filename):
		...
		f.extractall()
		f.close()
		data=xlrd.open_workbook('f_year.xls')
		table=data.sheets()[0]
		lexcel=[]
		...
		if os.path.exists('f_year.xls'):
			os.remove('f_year.xls')
