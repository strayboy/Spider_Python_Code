...
class usda_refresh():
	...
	def parse(self):
		...
		description=a[5]
		if(a[3]==1):
			self.filename=str(description).split(";")[-2]
			self.parse_txt()
	...
	def parse_txt(self):
		...
		for line in f:
			...
			if table==0:
				...
				try:
					...
					cbot=cur.fetchone()
					description=cbot[5]
					description=description+"Weekly入库;"
					...
					cur.execute(sql,(publish_date,python_time_str,description,1))
					try:
						conn.commit()
					except:
						conn.rollback()
					...
					description=description+"Weekly读取出错"
					python_time_str = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
				...
				try:
					...
					if(str(description)==None):
						description="Country error;"+self.filename+";"
					else:
						description=str(description)+"Country error;"+self.filename+";"
					
					python_time_str = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
					sql="update SYS_GlabFlag set PythonTime = %s,Description = %s,PythonFlag = %s where Module = '出口检验'"
					self.cur.execute(sql,(python_time_str,str(description),0))
					try:
						self.conn.commit()
					except:
						self.conn.rollback()
				...
				try:
					...
					description=self.cur.fetchone()[5]
					if(str(description)==None):
						description="Collections error;"+self.filename+";"
					else:
						description=str(description)+"Collections error;"+self.filename+";"
		...
		description=cbot[5]
		if(str(description)==None):
			description="Country success;"+self.filename+";"
		else:
			description=str(description)+"Country success;"+self.filename+";"
		...
		sql="insert into AMS_Inspected_Corrections values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
		try:
			self.cur.executemany(sql,rctablesix)
			self.conn.commit()
		except:
			conn.rollback()
			sql="select * from SYS_GlabFlag where Module='出口检验'"
		...
		description=cbot[5]
		if(str(description)==None):
			description="Collections sucess;"+self.filename+";"
		else:
			description=str(description)+"Collections success;"+self.filename+";"
