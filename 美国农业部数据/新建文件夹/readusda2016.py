...
class usda_refresh(scrapy.Spider):
	...
	def parse_txt(self,response):
		f=open(response.url.split("/")[-1].split(".")[-2]+time.strftime("%Y%m%d",time.localtime(time.time()))+".txt","w")
		f.write(response.body)
		f.close()
		time.sleep(3)
		f=open(response.url.split("/")[-1].split(".")[-2]+time.strftime("%Y%m%d",time.localtime(time.time()))+".txt","rU")
		table=0
		unit=""
		end=0
		head=[]
